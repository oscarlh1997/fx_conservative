...
class SystemDiagnostics:
    ...
    def _analyze_order_execution(self):
        ...
        try:
            ...
            orders_df["response"] = orders_df["order_response_json"].apply(_parse_resp)

            # Detect rejected/errored orders - Alpaca uses "status" field
            # A rejected order has status "rejected" or contains an "message"/"code" error key.
            def _is_rejected(r: dict) -> bool:
                status = str(r.get("status", "")).lower()
                if status in ("rejected", "canceled", "cancelled"):
                    return True
                # Alpaca error responses include a 'message' key at top level
                if "message" in r and "code" in r:
                    return True
                return False

            rejected = orders_df["response"].apply(_is_rejected).sum()
            ...
            if rejection_rate > 0.1:
                self.issues.append(DiagnosticIssue(
                    ...
                    description=f"Tasa de rechazo: {rejection_rate:.1%} (máx recomendado: 10%)",
                    impact="Señales no ejecutándose, pérdida de oportunidades",
                    recommendation=(
                        "Revisar validación de SL/TP y units en place_bracket_market(). "
                        "Verificar que los símbolos en config.yaml sean válidos en Alpaca."
                    ),
            ...
            if not fills_df.empty:
                ...
                if missing_fills:
                    self.issues.append(DiagnosticIssue(
                        ...
                        impact="Posibles fills no sincronizados con el estado local",
                        recommendation="Ejecutar sync_transactions() para sincronizar fills pendientes de Alpaca"
                    ))
    ...
    def _analyze_system_health(self):
        ...
        if missing_vars:
            self.issues.append(DiagnosticIssue(
                severity="CRITICAL",
                category="SYSTEM",
                title="Variables de entorno de Alpaca faltantes",
                description=f"Faltan: {', '.join(missing_vars)}",
                impact=impact_msg,
                recommendation=(
                    "Configurar ALPACA_API_KEY y ALPACA_SECRET_KEY como variables de entorno "
                    "o en un archivo .env en el directorio raíz del proyecto."
                ),
                metric_value=len(missing_vars)
            ))

        # Analizar errores en orders.csv (respuestas con error Alpaca-style)
        try:
            orders_df = self._load_orders()
            if not orders_df.empty:
                def _parse_resp(x):
                ...
                # Alpaca error: {"code": ..., "message": ...}
                def _is_error(r: dict) -> bool:
                    return "message" in r and "code" in r

                errors = orders_df["response"].apply(_is_error).sum()
                error_rate = errors / len(orders_df) if len(orders_df) > 0 else 0.0

                if error_rate > 0.05:
                    self.issues.append(DiagnosticIssue(
                        severity="HIGH",
                        category="SYSTEM",
                        title="Alta tasa de errores de API Alpaca",
                        description=f"{error_rate:.1%} de órdenes retornaron error de API",
                        impact="Problemas de conectividad, credenciales o validación",
                        recommendation=(
                            "Revisar logs para detalles. "
                            "Verificar credenciales Alpaca, límites de cuenta y símbolos válidos."
                        ),
    ...
    def _calculate_summary_metrics(self, lookback_days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not trades_df.empty:
                ...
                if "hold_days" in trades_df.columns and not trades_df["hold_days"].isna().all():
    ...
    def _load_trades(self, lookback_days: Optional[int] = None) -> pd.DataFrame:
        """Carga datos de trades y calcula hold_days si no existe la columna."""
        ...
        # Compute hold_days if missing or all NaN
        if "hold_days" not in df.columns or df["hold_days"].isna().all():
            try:
                df["hold_days"] = (df["exit_ts"] - df["entry_ts"]).dt.total_seconds() / 86400.0
            except Exception:
                df["hold_days"] = np.nan
