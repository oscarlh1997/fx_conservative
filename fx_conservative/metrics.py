...
def compute_trade_metrics(trades_csv: str) -> Dict:
    ...
    # Duración media: prefer hold_days column, compute from timestamps if missing/all-NaN
    if "hold_days" not in tr.columns or tr["hold_days"].isna().all():
        try:
            tr["hold_days"] = (tr["exit_ts"] - tr["entry_ts"]).dt.total_seconds() / 86400.0
        except Exception:
            tr["hold_days"] = np.nan

    out["AvgHoldDays"] = float(tr["hold_days"].mean()) if not tr["hold_days"].isna().all() else None
