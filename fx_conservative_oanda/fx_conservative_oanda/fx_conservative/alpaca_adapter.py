...
class AlpacaAdapter:
    """
    Minimal Alpaca adapter to plug into the existing strategy.

    Notes/assumptions:
    - This adapter targets Alpaca US equities via Trading API + Stock Market Data.
    - FX (EURUSD, etc.) is not supported by Alpaca Trading API. If you keep the
      default FX config, this adapter will not work for live trading.
    - Trailing/breakeven are implemented by replacing the existing stop order
      price (daily recalibration), not by a native trailing-stop distance.
    """
    ...
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                "Falta dependencia 'alpaca-py'. Instala requirements.txt antes de usar BROKER=alpaca."
            ) from e
        ...
        if not (api_key and secret_key):
            raise RuntimeError("Faltan variables de entorno ALPACA_API_KEY y/o ALPACA_SECRET_KEY")
    ...
    def candles(
        ...
        # Strategy expects daily bars. For equities, Alpaca "Day" bars are exchange-session aligned.
        if granularity not in ("D", "1D"):
            raise ValueError(f"Solo soportamos granularity diaria en AlpacaAdapter: {granularity}")

        # Request a wide-enough window for stock weekends/holidays.
        end = datetime.now(timezone.utc)
        start = end - timedelta(days=int(max(30, count * 3)))
        ...
        # alpaca-py returns a BarSet with a dataframe as .df
        df = getattr(bars, "df", None)
        if df is None or df.empty:
            return pd.DataFrame(columns=["Open", "High", "Low", "Close", "Volume"])

        # MultiIndex: (symbol, timestamp). Normalize into the strategy's expected OHLCV format.
        ...
        # Alpaca columns: open, high, low, close, volume
        ...
        out["Volume"] = df.get("volume", 0).astype(float).values
        return out
    ...
    def candles_between(
        ...
        # Reuse candles() and slice; good enough for offline/backtest usage.
        df = self.candles(instrument, granularity=granularity, count=5000, alignment_tz=alignment_tz, daily_hour=daily_hour, price=price)
    ...
    def place_bracket_market(
        ...
        # Bracket orders are supported for US equities. The stop leg is a stop order (not trailing distance).
        req = MarketOrderRequest(
            symbol=instrument,
            qty=int(units),
            side=order_side,
            # Bracket example in Alpaca docs uses gtc; keeps exit legs working beyond the session.
        ...
        # Normalize to dict for logger compatibility.
        if hasattr(o, "model_dump"):
            return o.model_dump()
        if hasattr(o, "dict"):
            return o.dict()
        return {"id": getattr(o, "id", None), "symbol": instrument}
